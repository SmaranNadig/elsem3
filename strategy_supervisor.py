# strategy_supervisor.py - LangChain Enhanced with Strategy Modes
    ...
    def _add_llm_insights(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df[needs_analysis].index:
            try:
                ...
                # Generate LLM strategy insight with mode context
                prompt = f"""You are a business strategist using {mode_config.name} strategy.

Strategy Focus: {mode_config.description}
Action Priorities: {', '.join([k for k, v in sorted(mode_config.action_priority_weights.items(), key=lambda x: x[1], reverse=True)[:3]])}
...
- Impact Score: {round(row['impact_score'], 0)}

Given the {mode_config.name} strategy, provide:
1) Why this action aligns with strategy
