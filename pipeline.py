# pipeline.py - Mode-Aware Agent Pipeline
...
def run_pipeline(
    ...
    # Load SKU strategy modes
    if verbose:
        print("[INFO] Loading SKU strategy modes...")
    sku_modes = sku_mode_manager.get_all_modes()
    if verbose:
        mode_count = len([m for m in sku_modes.values() if m != sku_mode_manager.default_mode])
        print(f"[INFO] Loaded {mode_count} custom mode assignments (default: {sku_mode_manager.default_mode})")

    # Agent 2: Profit Doctor (mode-aware)
    if verbose:
        print("[INFO] Running ProfitDoctorAgent (mode-aware)...")
    agent_start = time.time()
    profit_agent = ProfitDoctorAgent()
    df_profit = profit_agent.compute_profit_metrics(df_master, sku_modes=sku_modes)
    if verbose:
        print(f"[INFO] ProfitDoctor completed in {time.time() - agent_start:.2f}s")

    # Agent 3: Inventory Sentinel (mode-aware)
    if verbose:
        print("[INFO] Running InventorySentinelAgent (mode-aware)...")
    agent_start = time.time()
    inventory_agent = InventorySentinelAgent()
    df_inventory = inventory_agent.compute_inventory_metrics(df_profit, df_sales, sku_modes=sku_modes)
    if verbose:
    ...
    # Select columns for output (including LLM insights and strategy mode)
    cols_for_output = [
