...
const AnalyticsPage: React.FC = () => {
    ...
    const analyzeColumns = (products: ProductData[]) => {
        ...
        // Columns to EXCLUDE from KPI aggregation (non-meaningful numeric identifiers)
        const excludedCols = [
            'month', 'day_of_week', 'is_weekend', 'day', 'year', 'week',
            'id', 'code', 'index', 'row', 'lead_time_days'
        ];

        const numCols = Object.keys(firstItem).filter(key => {
            const val = firstItem[key];
            const isNumeric = typeof val === 'number' || (!isNaN(Number(val)) && val !== null && val !== '');
            const isExcluded = excludedCols.some(exc => key.toLowerCase().includes(exc.toLowerCase()));
            ...
            <div className="container mx-auto px-6 pt-12 pb-20">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12">
                    {numericCols.slice(0, 4).map((col, idx) => {
                        ...
                        // Check if this is a currency/price column
                        const isCurrency = col.toLowerCase().includes('price') ||
                        ...
                        const getColor = () => {
                            if (col.includes('profit')) return 'text-green-400';
                            if (col.includes('loss') || col.includes('cost')) return 'text-red-400';
                            if (col.includes('stock') || col.includes('units')) return 'text-blue-400';
                            return idx % 2 === 0 ? 'text-purple-400' : 'text-cyan-400';
                        };
                        ...
                        return (
                            ...
                            >
                                ...
                                <h3 className={`text-3xl font-bold mb-1 ${getColor()}`}>{formatValue(total)}</h3>
                                <p className="text-xs text-gray-500">Avg: {isCurrency ? '₹' : ''}{avg.toLocaleString(undefined, { maximumFractionDigits: 1 })}</p>
                            </motion.div>
                        );
                    })}
                </div>
