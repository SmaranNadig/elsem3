# ad_optimizer.py - LangChain-powered Ad Optimization Agent with Strategy Modes
...
class AdOptimizerAgent:
    ...
    def analyze_campaign(self, campaign: Dict[str, Any], strategy_mode: str = StrategyMode.BALANCED) -> Dict[str, Any]:
        """
        Analyze a single campaign and provide mode-aware recommendations
        """
        ...
        # Get mode-specific ROAS target
        mode_config = get_mode_config(strategy_mode)
        target_roas = mode_config.ad_roas_target
        min_roas = target_roas * 0.5  # Minimum is half of target
        ...
        result = {
            ...
            "roas": roas,
            "roas_gap": round(target_roas - roas, 2),
            "issues": issues,
            "llm_insight": "",
        ...
        if self.has_llm and performance in ["UNDERPERFORMING", "CRITICAL"]:
            result["llm_insight"] = self._generate_campaign_insight(campaign, strategy_mode, target_roas)
        
        return result
    
    def _generate_campaign_insight(self, campaign: Dict[str, Any], strategy_mode: str = StrategyMode.BALANCED, target_roas: float = 3.0) -> str:
        """Generate mode-aware LLM insight for a campaign"""
        mode_config = get_mode_config(strategy_mode)
        
        try:
            prompt = f"""You are an ad optimization expert using {mode_config.name} strategy.

Strategy Focus: {mode_config.description}
ROAS Target: {target_roas:.1f}x (mode-specific)
Ad Spend Aggressiveness: {mode_config.ad_spend_aggressiveness}
...
- ROAS: {campaign.get('roas', 0):.2f}x (Target: {target_roas:.1f}x)
