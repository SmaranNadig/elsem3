# inventory_sentinel.py - LangChain Enhanced with Strategy Modes
...
class InventorySentinelAgent:
    ...
    def compute_inventory_metrics(
        ...
        Now mode-aware: adjusts safety stock and risk thresholds per mode
        """
        ...
        # Get mode-specific safety stock multipliers
        if sku_modes:
            df["strategy_mode"] = df["sku_id"].map(sku_modes).fillna(StrategyMode.BALANCED)
        else:
            df["strategy_mode"] = StrategyMode.BALANCED
        
        # Apply mode-specific safety stock multipliers
        df["safety_stock_multiplier"] = df["strategy_mode"].apply(
            lambda mode: get_mode_config(mode).safety_stock_multiplier
        )
        ...
        # Risk classification
        # Stock-based risk
        critical_stock = df["days_of_stock_left"] < df["lead_time_days"]
        warning_stock = (
        ...
        # Profit-based risk: Negative profit per unit is CRITICAL
        negative_profit = df["profit_per_unit"] < 0
        
        # Combined risk classification
        critical = critical_stock | negative_profit

        df["risk_level"] = "SAFE"
        df.loc[warning_stock, "risk_level"] = "WARNING"
        df.loc[critical, "risk_level"] = "CRITICAL"
        df.loc[no_history & ~negative_profit, "risk_level"] = "NO_HISTORY"  # Negative profit takes priority

        # Reorder quantity (mode-aware)
        should_reorder = (
        ...
        df.loc[should_reorder, "reorder_qty_suggested"] = (
            ...
            * df.loc[should_reorder, "safety_stock_multiplier"]  # Mode-aware multiplier
        ).astype(int)
    ...
    def _add_llm_insights(self, df: pd.DataFrame, sku_modes: dict = None) -> pd.DataFrame:
        ...
        for idx in df[needs_analysis].index:
            try:
                ...
                prompt = f"""You are an inventory expert using {mode_config.name} strategy.

Strategy Focus: {mode_config.description}
Risk Tolerance: {mode_config.risk_tolerance}
Safety Stock Multiplier: {mode_config.safety_stock_multiplier}x
Stockout Penalty Weight: {mode_config.stockout_penalty_weight}
...
1) Risk assessment aligned with strategy
2) Demand pattern analysis
3) Reorder strategy recommendation
