# seasonal_analyst.py - LangChain Enhanced Seasonal Analysis Agent with Strategy Modes
    ...
    def _add_llm_insights(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for idx in df[needs_analysis].index:
            try:
                ...
                prompt = f"""You are a seasonal business analyst using {mode_config.name} strategy.

Strategy Focus: {mode_config.description}
Seasonal Forecast Conservatism: {mode_config.seasonal_forecast_conservatism}
Risk Threshold: {mode_config.seasonal_risk_threshold}
...
1) Inventory action for upcoming season
2) Pricing strategy aligned with mode
3) Key timing to watch

Be concise and strategy-focused (under 100 words)."""
