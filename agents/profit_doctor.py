# profit_doctor.py - LangChain Enhanced with Strategy Modes
...
class ProfitDoctorAgent:
    ...
    def compute_profit_metrics(self, df_master: pd.DataFrame, sku_modes: dict = None) -> pd.DataFrame:
        """
        Compute profit metrics with optional LLM enhancement
        
        Args:
            df_master: Master SKU dataframe
            sku_modes: Dict mapping sku_id to strategy mode (optional)
        """
        ...
        try:
            ...
            if CFG.enable_ad_gateway:
                ...
                if "ad_spend_total_last_90_days" in df.columns:
                    df["ad_spend_30d"] = np.where(
                        df["ad_spend_30d_gateway"] > 0,
                        df["ad_spend_30d_gateway"],
                        df["ad_spend_total_last_90_days"] / 3.0 # Approx 30d spend from 90d
                    )
                else:
                ...
                df["ad_cost_per_unit"] = np.divide(
                    df["ad_spend_30d"],
                    df["units_sold_last_90_days"] / 3.0, # Approx 30d units from 90d
                    out=np.zeros_like(df["ad_spend_30d"], dtype=float),
                    where=df["units_sold_last_90_days"] > 0
                )
            ...
            if "ad_spend_total_last_90_days" in df.columns:
                df["ad_cost_per_unit"] = np.divide(
                    df["ad_spend_total_last_90_days"],
                    df["units_sold_last_90_days"],
                    out=np.zeros_like(df["ad_spend_total_last_90_days"], dtype=float),
                    where=df["units_sold_last_90_days"] > 0
                )
            else:
        ...
        # Units sold per day
        df["units_sold_per_day"] = df["units_sold_last_90_days"] / 90.0
        ...
        # Add mode-aware margin flags
        if sku_modes:
            df["strategy_mode"] = df["sku_id"].map(sku_modes).fillna(StrategyMode.BALANCED)
            df = self._add_mode_aware_flags(df)
        else:
            df["strategy_mode"] = StrategyMode.BALANCED
        
        # STEP 2: LLM Enhancement (optional - adds insights)
        if self.has_llm and CFG.enable_profit_doctor_llm:
            df = self._add_llm_insights(df, sku_modes)
        
        return df
    
    def _add_mode_aware_flags(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Add mode-aware margin flags based on strategy mode thresholds
        """
        df["is_low_margin"] = False
        df["margin_alert_level"] = "OK"
        
        for idx, row in df.iterrows():
            mode = row["strategy_mode"]
            config = get_mode_config(mode)
            
            margin_pct = row["profit_per_unit"] / row["selling_price"] if row["selling_price"] > 0 else 0
            
            # Check against mode-specific thresholds
            if margin_pct < config.min_margin_threshold:
                df.at[idx, "is_low_margin"] = True
                df.at[idx, "margin_alert_level"] = "CRITICAL"
            elif margin_pct < config.low_margin_alert_threshold:
                df.at[idx, "is_low_margin"] = True
                df.at[idx, "margin_alert_level"] = "WARNING"
        
        return df
    
    def _add_llm_insights(self, df: pd.DataFrame, sku_modes: dict = None) -> pd.DataFrame:
        """
        Add LLM-generated insights to profitable/loss-making products
        Now includes strategy mode context in prompts
        ...
        for idx in df[needs_analysis].index:
            try:
                row = df.loc[idx]
                mode = row.get("strategy_mode", StrategyMode.BALANCED)
                mode_config = get_mode_config(mode)
                
                # Generate LLM insight with mode context
                prompt = f"""You are a profit optimization expert using {mode_config.name} strategy.

Strategy Focus: {mode_config.description}
Risk Tolerance: {mode_config.risk_tolerance}
Min Margin Target: {mode_config.min_margin_threshold * 100}%
...
Given the {mode_config.name} strategy, provide:
1) Brief profitability assessment
2) Pricing recommendation aligned with strategy
3) Cost optimization tip

Be concise and strategy-focused."""
